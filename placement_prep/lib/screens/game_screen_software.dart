...
import 'package:placement_prep/utilities/alphabet.dart';
import 'package:placement_prep/components/word_button.dart';
...
import 'package:placement_prep/utilities/score_db.dart' as score_database;
import 'package:placement_prep/utilities/user_scores.dart';

//reference : https://www.youtube.com/watch?v=rMCGttgJoDI&t=3s

class SoftGameScreen extends StatefulWidget {
  SoftGameScreen({@required this.softObject});

  final SoftHangmanWords softObject;

  @override
  _SoftGameScreenState createState() => _SoftGameScreenState();
}

class _SoftGameScreenState extends State<SoftGameScreen> {
  ...
  void initWords() {
    ...
    wordList = [];
    hintLetters = [];
    word = widget.softObject.getWord();
//    print
    print('this word is ' + word);
    if (word.length != 0) {
      hiddenWord = widget.softObject.getHiddenWord(word.length);
    } else {
      returnHomePage();
    }
