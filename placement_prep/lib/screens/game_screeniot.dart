...
class IotGameScreen extends StatefulWidget {
  IotGameScreen({@required this.iotObject});

  final IotHangmanWords iotObject;

  @override
  _IotGameScreenState createState() => _IotGameScreenState();
}

class _IotGameScreenState extends State<IotGameScreen> {
  ...
  void initWords() {
    ...
    wordList = [];
    hintLetters = [];
    word = widget.iotObject.getWord();
//    print
    print('this word is ' + word);
    if (word.length != 0) {
      hiddenWord = widget.iotObject.getHiddenWord(word.length);
    } else {
      returnHomePage();
    }
